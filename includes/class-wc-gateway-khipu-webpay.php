...
class WC_Gateway_khipu_webpay extends WC_Gateway_khipu_abstract
{
    ...
    public function __construct()
    {
        ...
        $this->method_title = __('khipu WebPay', 'woocommerce-gateway-khipu');
        ...
        $logo = $this->get_payment_method_icon('WEBPAY_PSP');
        if(!$logo) {
            $logo = $this->get_payment_method_icon('WEBPAY');
        }
        $this->icon = $logo;
    ...
    {
        return $this->is_payment_method_available_in_khipu("WEBPAY") || $this->is_payment_method_available_in_khipu("WEBPAY_PSP");
    }
        ...
        $this->form_fields = array(
            ...
            'title' => array(
                ...
                'description' => __('This controls the title which the user sees during checkout.',
                    'woocommerce'),
                'default' => __('khipu - WebPay Crédito y Débito', 'woocommerce-gateway-khipu', 'woocommerce-gateway-khipu'),
                'desc_tip' => true
            ),
            'description' => array(
                ...
                'description' => __('Payment method description that the customer will see on your checkout.',
                    'woocommerce'),
                'default' => __('Paga con cualquier Tarjeta de Crédito o RedCompra.', 'woocommerce-gateway-khipu')
            ),
        );
    ...
    function receipt_page($order)
    {
        ...
        if (method_exists($response, 'getWebpayUrl')) {
            wp_redirect($response->getWebpayUrl());
        } else {
            echo $response;
        }
    }
